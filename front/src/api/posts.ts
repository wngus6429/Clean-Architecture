 ...
 * 페이지네이션 게시글 조회
 */
export async function getPostsPage(params: { page: number; pageSize: number }): Promise<{
  items: Post[];
  total: number;
  page: number;
  pageSize: number;
}> {
  const query = new URLSearchParams({ page: String(params.page), pageSize: String(params.pageSize) }).toString();
